...
def merge_clusters(base_dir="negotiation"):
    """
...
# Part 2: Update original file to add Alice's clustering labels
def add_alice_labels(merge_file="negotiation/merged_clusters.jsonl",
                    origin_file="negotiation_with_formatted_ids.jsonl",
                    updated_file="llama3-8b_negotiation_with_alice_labels.jsonl"):
    """
    Add clustering labels to Alice's actions
    ...
    """
    print(f"Starting to add clustering labels for Alice...")
    ...
    with open(origin_file, 'r') as infile, open(updated_file, 'w') as outfile:
        for line in infile:
            ...
            for k in range(2, 101):
                alice_labels_by_k[k] = [
                    merge_data[k].get(action_id[1:], None)  # Remove prefix 'b' from action_id for lookup
...
# Part 3: Update original file to add Bob's clustering labels
def add_bob_labels(merge_file="negotiation/merged_clusters.jsonl",
                  alice_file="llama3-8b_negotiation_with_alice_labels.jsonl",
                  updated_file="llama3-8b_negotiation_with_labels_k2_to_k100.jsonl"):
    """
    Add clustering labels to Bob's actions
    
    Args:
        merge_file: Path to merged clustering data file
        alice_file: Path to file with Alice labels added
        updated_file: Path to final updated output file
    """
    print(f"Starting to add clustering labels for Bob...")
    ...
    # Open file with Alice labels, process line by line and update
    with open(alice_file, 'r') as infile, open(updated_file, 'w') as outfile:
        for line in infile:
            data = json.loads(line.strip())
...
def main():
    print("Starting negotiation data clustering post-processing workflow...")
    
    # Customizable base directory and file paths
    base_dir = "negotiation"
    origin_file = "negotiation_with_formatted_ids.jsonl"
    
    # Step 1: Merge clustering results
    merge_file = merge_clusters(base_dir)
    
    # Step 2: Add Alice labels
    alice_file = add_alice_labels(merge_file, origin_file)
    
    # Step 3: Add Bob labels
    final_file = add_bob_labels(merge_file, alice_file)
